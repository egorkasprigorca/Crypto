...
def mental_poker(players_num):
    ...
    for i in range(players_num):
        deck_keys = [binaryPow(j, C[i], p) for j in deck_keys]
    ...
    for i in range(players_num):
        table = [binaryPow(table[j], D[i], p) for j in range(len(table))]
    ...
    for i in range(players_num):
        for j in range(players_num):
            if i != j:
                v = 0
                while v < len(hands[0]):
                    hands[i][v] = binaryPow(hands[i][v], D[j], p)
                    v += 1
        v = 0
        while v < len(hands[0]):
            hands[i][v] = binaryPow(hands[i][v], D[i], p)
            v += 1
